...
def cal_eval(y_real, y_pred):
    ...
    return df_eval
df = pd.read_csv('data\\test_data_多变量.csv',encoding='gbk')
...
class Config:
    def __init__(self):
        ...
        self.patch_len = 16  # GTR参数 patch len for patch_embedding
        self.stride = 8  # GTR参数 stride for patch_embedding
        ...
        self.beta=1
        self.cycle=8
        self.model_type='mlp'#'model type, options: [linear, mlp]
        self.use_revin=1
        self.individual=0
        #self.
        #self.
...
model_type = 'GTR'
net = GTR.Model(config).to(device)
...
result_df.to_csv('GTR真实值与预测值.csv', index=False, encoding='utf-8')
# 打印保存成功的消息
