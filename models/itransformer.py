...
# x_mark是用来表示要不要加入时序嵌入
# 倒置嵌入是将时间维度L映射到d_model
class DataEmbedding_inverted(nn.Module):
    ...
    def forward(self, x, x_mark): # [B, L, M] and [B, L, 4] -> [B, M(+4), d_model]
        x = x.permute(0, 2, 1) # [B, L, M] -> [B, M, L]
        # x: [Batch Variate Time]
        if x_mark is None:
            x = self.value_embedding(x) # [B, M, L] -> [B, M, d] 注: M是变量数, d即为d_model
        else:
            x = self.value_embedding(torch.cat([x, x_mark.permute(0, 2, 1)], 1)) # [B, M+4, L] -> [B, M, d]
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, x_mark): # [B, L, M] and [B, L, 4] -> [B, L, d]
        ...
        if x_mark is None:
            x = self.value_embedding(x) # [B, L, M] -> [B, L, d]
        else:
            x = self.value_embedding(x) + self.mark_embedding(x_mark) # [B, L, M] -> [B, L, d] and [B, L, 4] -> [B, L, d]
        # x: [Batch Variate d_model]
        return self.dropout(x)
...
class Backbone1(nn.Module):
    def __init__(self, input_size, output_size, d_model, enc_in, factor, dropout, output_attention, n_heads, d_ff, activation, e_layers):
        ...
        # self.predict = nn.Linear(d_model*enc_in, output_size)
    ...
    def forward(self, x): # [B, L, M] and [B, L, 4] -> [B, H]
        ...
        # iTransformer
        # iTransformer中只有值嵌入,没有位置嵌入和时序嵌入
        x_embed = self.embed(x, None) # [B, L, M] and [B, L, 4] -> [B, M(+4), d_model]
        y, _ = self.encoder(x_embed) # [B, M(+4), d_model] -> [B, M(+4), d_model]
        y = y[:, 0, :] # [B, M(+4), d_model] -> [B, d_model]
        # y = self.flatten(y) # [B, M, d_model] -> [B, M*d_model]
        y = self.predict(y) # [B, d_model] -> [B, H]
        # y = self.predict(y).permute(0, 2, 1)[:, :, :D] # [B, M(+4), d] -> [B, d, M(+4)] -> [B, H, M(+4)] 原来代码里面有的

        # encoder+MLP的Transformer
        # x_embed = self.embed2(x, batch_x_mark)  # [B, L, M(+4)] -> [B, L, d]
...
# mymodel = iTransformer(104,16,128,4,5,0.01,False,4,256,'relu',1)
...
class Model(nn.Module):
    def __init__(self, configs):
        super().__init__()
        self.model = iTransformer(configs.seq_len, configs.pred_len, 128,
